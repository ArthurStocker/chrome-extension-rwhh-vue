<template>
  <b-table striped hover :name="name" :fields="definition" :items="records" :filter="filter" selectable select-mode="single" selectVariant="info" @row-selected="select">
    <template v-for="key in definition.map((field) => field.key)" :slot="key" slot-scope="records">
      <input class="form-control" v-if="editable" v-bind:key="key" :data-dataset="name" :data-row="records.index" :data-col="key" :type="records.field.type" :value="valueTransformer('text', key, records)" :checked="valueTransformer('checkbox', key, records)" @input="storeCommitEvent" />
