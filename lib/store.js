...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        editRecord: (state, data) => {
            ...
            state.datasets.filter((dataset) => dataset.name == data.datasetname)[0].definition.filter((field) => field.key == data.col)[0].editable = data.value || data.row 
        },
        selections: (state, data) => { Vue.set(state.selections, data.key, data.value) },
        editable: (state, data) => { state.editable = data || !state.editable }
    }
})
